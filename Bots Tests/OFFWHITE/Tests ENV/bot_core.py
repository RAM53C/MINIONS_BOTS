...
def getSource():
    ...
    logToConsole("Waiting for response...")
    response = waitfor_image("imagedata/html.jpeg")
    if response:
        logToConsole("Received Response")
    # Get html
...
def RandomRequest():
    ...
    if pmodelsset == False:
        ...
        #Get link
        loadLink(str(CAT), "imagedata/offwhite.jpeg")
        # Get HTML
        page_html = getSource();
        # Parse html
    ...
    #Get link
    loadLink(str(randlink), "imagedata/offwhite.jpeg")
    i = i + 1

def LinkRequest(link):
    #Get link
    loadLink(str(link), "imagedata/offwhite.jpeg")
    # Get product state (Using OpenCV)
    product_available = detect_image("imagedata/addtocart.jpeg")
    #Strip Link
