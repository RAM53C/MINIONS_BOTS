...
function cliconsole() {
  ...
  io2.on('connection', client2 => {
      //New connection
      var clientIp2 = client2.request.connection.remoteAddress;
      console.log("[MOBILE]: New connection from " + clientIp2)

      client2.on('mobile_cmd', data => {
    ...
    client2.on('disconnect', () => {
      console.log("[MOBILE]: Client " + clientIp2 + " disconnected")
    });
  });
  console.log("[MOBILE]: Waiting for connections...");
  server2.listen(3008);
}
